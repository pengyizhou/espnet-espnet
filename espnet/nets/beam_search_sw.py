...
class BeamSearch(torch.nn.Module):
    ...
    def __init__(
        ...
        penalty_mode: int = None,
        penalty_ratio: float = None,
    ...
    def score_full(
        ...
        for k, d in self.full_scorers.items():
            ...
            if k == 'lm':
                ...
                yseq = hyp.yseq.clone()
                if last_sw_id:
                    yseq = torch.cat((torch.tensor([self.sos]), yseq[last_sw_id[0] + 1: ]), 0)
                    # logging.info("#### [LOG-score_rnnlm] last_sw_id is {}".format(last_sw_id))
                if self.penalty_mode:
                    # if self.penalty_mode == "1": # do nothing to SW label
                    # logging.info("#### [LOG-mode1] yseq is {}".format(yseq))
                    scores[k], states[k] = d.score(yseq, hyp.states[k], x)
                    # elif self.penalty_mode == "2": # give 0 score for SW label
                    #    logging.info("#### [LOG-mode2] yseq is {}".format(yseq))
                    #    scores[k], states[k] = d.score(yseq, hyp.states[k], x)
                        # switching_labels_index = torch.eq(self.switching_label_id).nonzero().tolist()
                        # scores[k][switching_labels_index] = 0
    ...
    def score_partial(
        ...
        for k, d in self.part_scorers.items():
            ...
            if k == 'ngram':
                ...
                if last_sw_id:
                    yseq = torch.cat((torch.tensor([self.sos]), yseq[last_sw_id[0] + 1: ]), 0)
                    # logging.info("#### [LOG-score_partial] last_sw_id is {}".format(last_sw_id))
                if self.penalty_mode:
                    if self.penalty_mode == "1": # do nothing to SW label
                        # logging.info("#### [LOG-mode1] yseq is {}".format(yseq))
                        ...
                        # logging.info("#### [LOG-mode2] yseq is {}".format(yseq))
                        scores[k], states[k] = d.score_partial(yseq, ids, hyp.states[k], x) 
                        switching_labels_index = torch.eq(ids, self.switching_label_id).nonzero().tolist()
                        scores[k][switching_labels_index] = 0
                    elif self.penalty_mode == "3":
                        scores[k], states[k] = d.score_partial(yseq, ids, hyp.states[k], x)
                    elif self.penalty_mode == "4":
                        scores[k], states[k] = d.score_partial(yseq, ids, hyp.states[k], x)
                        switching_labels_index = torch.eq(ids, self.switching_label_id).nonzero().tolist()
                        scores[k][switching_labels_index] += self.award_score
                        if switching_labels_index:
                            if scores[k][switching_labels_index] > 0:
                                scores[k][switching_labels_index] = 0
            else: # for ctc scorer
                scores[k], states[k] = d.score_partial(hyp.yseq, ids, hyp.states[k], x)
        return scores, states
    ...
    def merge_states(self, states: Any, part_states: Any, part_idx: int, is_sw: bool) -> Any:
        ...
        for k, d in self.full_scorers.items():
            ...
            if is_sw == True and k == 'lm' and self.penalty_mode:
                new_states[k] = d.init_state(torch.zeros(1))
                # logging.info("#### [LOG] reinitialize states")
        for k, d in self.part_scorers.items():
            new_states[k] = d.select_state(part_states[k], part_idx)
            if is_sw == True and k == 'ngram' and self.penalty_mode:
                new_states[k] = d.init_state(torch.zeros(1))
                # logging.info("#### [LOG] reinitialize states")
        return new_states

    def punish_scores_ngram(self, 
                            scores: torch.Tensor,
        ...
        Args:
            scores(torch.Tensor)
                  weighted scores from full scorers, e.g. decoder/rnnlm
            part_scores(Dict[str, Union[float, torch.Tensor]])
        ...
        """
        # ipdb.set_trace()
        new_part_scores = part_scores
        if self.switching_label_id in part_ids:
            switching_label_index = torch.eq(part_ids, self.switching_label_id).nonzero()[-1].tolist()
            # ipdb.set_trace()
            weight_scores = scores[part_ids].clone()
            weight_scores += self.weights['ctc'] * new_part_scores['ctc'] # combine CTC score
            # if  weight_scores[switching_label_index] != torch.max(weight_scores):
            # if <sw> do not have the highest weighted score by decoder+rnnlm+ctc
            # then give it a higher score by ngram lm
            switching_label_ngram_score = new_part_scores['ngram'][switching_label_index]
            max_ngram_score = torch.max(new_part_scores['ngram'])
            assert 0 <= self.penalty_ratio <= 1, 'penalty ratio should be float 0~1'
            new_switching_label_ngram_score = (max_ngram_score - switching_label_ngram_score) * self.penalty_ratio + switching_label_ngram_score
            new_part_scores['ngram'][switching_label_index] =  float(new_switching_label_ngram_score)
                
        return new_part_scores
    ...
    def search(
        ...
        for hyp in running_hyps:
            ...
            # if self.penalty_mode == "2" and "lm" in self.full_scorers:
            #    switching_labels_index = torch.eq(part_ids, self.switching_label_id).nonzero().tolist()
            #    logging.info("##### [LOG-penalty-rnnlm] switching_labels_index is {}".format(switching_labels_index[0]))
            ...
            # logging.info("part_score is {}".format(part_scores['ngram']))
            if self.penalty_mode == "3" and self.penalty_ratio:
                part_scores = self.punish_scores_ngram(weighted_scores, part_scores, part_ids)
                # logging.info("part_score changed is {}".format(part_scores['ngram']))
            # part_scores = self.punish_scores_ngram(part_scores, part_ids)
            language = hyp.language
            ...
            for j, part_j in zip(*self.beam(weighted_scores, part_ids)): # get new top 10 token id after add partial scores, and corresponding id in part_ids
                ...
                # logging.info(part_j)
                # logging.info("#### [LOG-search] j is {}".format(j))
                best_hyps.append(
                    Hypothesis(
