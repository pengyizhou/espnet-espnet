...
from espnet.nets.pytorch_backend.maskctc.add_mask_token_modified import mask_uniform_for_cs_coupling
...
class E2E(E2ETransformer):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        self.calcute_mwer = Calcute_Minmum_Word_Error_Rate(4)
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        logging.warning("## training status-122 is {}".format(self.training))
        
        if not self.training:
            logging.warning("## This batch is from valid so training status is {}".format(self.training))
            isValid = True
        else:
            isValid = False
        mwer_loss = 0
        for i in range(xs_pad.shape[0]):
            with torch.no_grad():
                feature = xs_pad[i].clone()
                best_scores, best_ids = self.recognize_mwer(feature)

            mwer_loss += self.calcute_mwer.get_Mwer_loss_recognize_positive(best_scores, best_ids, ys_pad[i])
        logging.warning("mwer_loss for one batch: {}".format(mwer_loss))
        logging.warning("End train for one batch")
        logging.warning("## training status-147 is {}".format(self.training))
        if not isValid:
            self.train(True)
        
        logging.warning("## training status-149 is {}".format(self.training))
        ...
        # )
        ext_mask_method = "BM"
        ys_in_pad, ys_out_pad, ys_in_coup, ys_out_coup = mask_uniform_for_cs_coupling(
        ...
        logging.warning("## training status-160 is {}".format(self.training))
        ...
        logging.warning("## training status-166 is {}".format(self.training))
        ...
        logging.warning("## training status-173 is {}".format(self.training))
        ...
        logging.warning("## training status-186 is {}".format(self.training))
        # 5. compute cer/wer
        ...
        logging.warning("## training status-193 is {}".format(self.training))
        alpha = self.mtlalpha
        if alpha == 0:
            ...
            self.loss = 0.01 * (alpha * loss_ctc + (1 - alpha) * (loss_att + loss_att_coup)) + mwer_loss
            loss_mwer_data = float(mwer_loss)
            loss_att_data = float(loss_att + loss_att_coup)
    ...
    def recognize_mwer(self, x):
        ...
        for i, y in enumerate(y_hat.tolist()):
            ...
            while cnt < ctc_ids.shape[1] and y == ctc_ids[0][cnt]:
                ...
                cnt += 1
        probs_hat = torch.from_numpy(numpy.array(probs_hat))
        p_thres = self.maskctc_probability_threshold
        mask_idx = torch.nonzero(probs_hat[y_idx] < p_thres).squeeze(-1)
        confident_idx = torch.nonzero(probs_hat[y_idx] >= p_thres).squeeze(-1)
        ...
        # iterative decoding
        if not mask_num == 0:
            K = self.maskctc_n_iterations   # 10
            num_iter = K if mask_num >= K and K > 0 else mask_num
            # logging.warning("K: {}; num_iter: {}".format(K, num_iter))
            for t in range(num_iter - 1):
