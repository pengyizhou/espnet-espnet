...
from espnet.nets.pytorch_backend.maskctc.add_mask_token import coupling_mask_uniform
...
class E2E(E2ETransformer):
    ...
    def forward(self, xs_pad, ilens, ys_pad, epoch):
        """E2E forward.
        ...
        self.hs_pad = hs_pad
        # ipdb.set_trace()
        logging.warning("##### LOG current epoch is {}".format(epoch))
        # 2. forward decoder
        ys_in_pad, ys_out_pad, ys_in_coup, ys_out_coup = coupling_mask_uniform(
        ...
        if alpha == 0:
            ...
            loss_att_data = float(loss_att) + float(loss_att_coup)
            loss_ctc_data = None
        else:
