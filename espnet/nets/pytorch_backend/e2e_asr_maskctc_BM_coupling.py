...
class E2E(E2ETransformer):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        alpha = self.mtlalpha
        if alpha == 0:
            self.loss = loss_att + loss_att_coup
            loss_att_data = float(loss_att + loss_att_coup)
            loss_ctc_data = None
        else:
            self.loss = alpha * loss_ctc + (1 - alpha) * (loss_att + loss_att_coup)
            loss_att_data = float(loss_att + loss_att_coup)
    ...
    def recognize(self, x, recog_args, char_list=None, rnnlm=None):
        ...
        logging.warning("for threshold {}: confident_idx is {} and mask_num is {}".format(p_thres, confident_idx, mask_num))
        """
        p_thres = 0.99
        mask_idx = torch.nonzero(probs_hat[y_idx] < p_thres).squeeze(-1)
        confident_idx = torch.nonzero(probs_hat[y_idx] >= p_thres).squeeze(-1)
        ...
        # logging.warning("ctc:{}".format(n2s(y_in[0].tolist())))
