...
class BeamSearch(torch.nn.Module):
            ...
            if k == 'ngram' or k == 'lm':
                ...
                if self.fusion_penalty:
                    if self.fusion_penalty == 'MAN':
                        ...
                        logging.info("Cleaning states while switching to another language")
    ...
    def punish_scores_ngram(self, 
        ...
        if self.switching_label_id in part_ids:
            ...
            if switching_label_ngram_score < -1:
                new_part_scores['ngram'][switching_label_index] = switching_label_ngram_score + 1
            else:
    ...
    def merge_states(self, states: Any, part_states: Any, part_idx: int, is_sw: bool) -> Any:
        ...
        for k, d in self.part_scorers.items():
            ...
            if is_sw == True and k == 'ngram' and self.fusion_penalty:
                logging.info("Clean previous state for {}".format(k))
            ...
            for j, part_j in zip(*self.beam(weighted_scores, part_ids)): # get new top 10 token id after add partial scores, and corresponding id in part_ids
                ...
                if len(hyp.yseq) == 1:
                    if hyp.yseq == self.sos:
                        if j <= self.switching_id: # the first hyp token is Man
                            language = 0
                        elif j > self.switching_id and j!= self.switching_label_id: # the first hyp token is Sge
                            language = 1
