...
def get_parser():
    ...
    parser.add_argument(
        "--penalty-lm",
        type=float,
        default=1.0, # 0~1 or -1
        help="Penalty ratio for ngram lm. 1 means no penalty, 0 means max penalty, -1 means highest score for <sw>",
    )
    parser.add_argument(
        "--result-label",
        type=str,
    ...
    if args.num_spkrs == 1:
        if args.backend == "chainer":
            ...
            if args.num_encs == 1:
                ...
                if args.api == "v2":
                    from espnet.asr.pytorch_backend.recog_multi_lm_penalty_unsw import recog_v2
