...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None, # 1/2/3
        help="scoring mode for switching token "
        "For ngram LM"
        "1: Remaining score for <sw> label "
        "2: Highest score for <sw> label "
        "3: Regarding highest score, ratio based penalty "
        "For RNNLM "
        "1: Remaining score for <sw> token "
        "2: If <sw> in top15 score of decoder, make it highest "
        "3: If <sw> in top15 score of decoder, Regarding highest score, ratio based penalty.",
    )

    parser.add_argument(
        "--penalty-ratio",
        type=float,
        default=None, # Float between 0~1
        help="penalty ratio for switching token while in penalty-mode 3.",
    )
    ...
    parser.add_argument(
        ...
        help="award score for penalty-mode 4.",
    )
    ...
    if args.num_spkrs == 1:
        if args.backend == "chainer":
            ...
            if args.num_encs == 1:
                ...
                if args.api == "v2":
                    from espnet.asr.pytorch_backend.recog_sw import recog_v2
